...
const VIOLATIONS: Record<string, ViolationRule> = {
  IMPORT_BOUNDARY: {
    ...
    description:
      'Enforce module isolation (External -> AI/index only; AI -> Lib or sibling only).',
  },
  DIRECT_FS_USAGE: {
    key: 'fs',
    name: 'Direct FS Usage',
    regex:
      /\bfs\.(?:readFile|writeFile|mkdir|readdir|stat|appendFile|unlink)(?:Sync)?\b/g,
    description:
      'Use standardized I/O helpers from src/lib/path.ts (readJson, writeJson, ensureDir, etc.) instead of raw fs calls.',
  },
  MANUAL_PATH_JOIN: {
    key: 'path',
    name: 'Manual Path Construction',
    regex:
      /\b(?:join|joinPaths|resolve|getAbsolutePath)\s*\(\s*(?:getAyniteDir\(\)|AYNITE_DIR|getAyniteConfigDir\(\)|getAynitePath\(\)|__dirname)\b/g,
    description:
      'Use specialized path helpers from lib/path.ts (e.g., getWorkspacesConfigPath, getAIConfigPath) instead of manual joining with base directories.',
  },
  STRICT_TYPING: {
    key: 'types',
  ...
  HARDCODED_STRINGS: {
    ...
    regex:
      /(?:content|prompt|message|description|name):\s*['"`]([^'"`]{50,})['"`]/g,
    description:
      'Move large prompt strings, tool metadata, or system messages to lib/constants/ai.ts.',
  },
  DIRECT_PATH_IMPORT: {
    key: 'path-import',
...
if (process.argv.includes('-h') || process.argv.includes('--help')) {
  ...
  console.log('Options:')
  console.log(
    '  --focus=[type]    Only run specific checks (import, fs, path, types, strings)',
  )
...
const auditFile = (filepath: string) => {
  ...
  if (activeViolations.some((v) => v.key === 'import')) {
    ...
    while ((match = importRegex.exec(content)) !== null) {
      ...
      if (importPath.startsWith('.')) {
        ...
        const currentSubsystem = SUBSYSTEMS.find((s) =>
          filepath.startsWith(s.dir),
        )
        const targetSubsystem = SUBSYSTEMS.find((s) =>
          resolvedPath.startsWith(s.dir),
        )
        ...
        if (!currentSubsystem) {
          ...
          if (targetSubsystem) {
            ...
            const targetIsIndex =
              resolvedPath === targetIndex ||
              resolvedPath === `${targetIndex}.ts` ||
              resolvedPath === targetSubsystem.dir
            if (!targetIsIndex) {
              violation = true
          ...
          const targetIsInsideSameSubsystem =
            targetSubsystem && targetSubsystem.dir === currentSubsystem.dir
          const _targetIsSibling =
            path.dirname(resolvedPath) === path.dirname(filepath)
  ...
  // 2. Direct FS Usage
  if (
    activeViolations.some((v) => v.key === 'fs') &&
    !filepath.endsWith('src/lib/path.ts')
  ) {
    let fsMatch
    while (
      (fsMatch = VIOLATIONS.DIRECT_FS_USAGE.regex.exec(content)) !== null
    ) {
  ...
  // 3. Manual Path Join
  if (
    activeViolations.some((v) => v.key === 'path') &&
    !filepath.endsWith('src/lib/path.ts')
  ) {
    let pathMatch
    while (
      (pathMatch = VIOLATIONS.MANUAL_PATH_JOIN.regex.exec(content)) !== null
    ) {
  ...
  if (activeViolations.some((v) => v.key === 'strings')) {
    let strMatch
    while (
      (strMatch = VIOLATIONS.HARDCODED_STRINGS.regex.exec(content)) !== null
    ) {
  ...
  // 8. Stale @ts-expect-error Comments
...
if (report.length === 0) {
  ...
  DISPLAY_ORDER.forEach((typeName) => {
    ...
    const badge =
      typeName === VIOLATIONS.IMPORT_BOUNDARY.name
        ? '🚨 ARCHITECTURE'
        : typeName === VIOLATIONS.STRICT_TYPING.name
          ? '🚨 TYPING'
          : typeName === VIOLATIONS.DIRECT_FS_USAGE.name
            ? '⚠️ WARNING'
            : typeName === VIOLATIONS.MANUAL_PATH_JOIN.name
              ? '⚠️ WARNING'
              : '📝 NOTICE'
