...
async function testProviders() {
  ...
  for (const [name, pConfig] of Object.entries(providers) as [string, any][]) {
    ...
    if (name !== 'ollama' && (!pConfig.apiKey || pConfig.apiKey.trim() === '')) {
    ...
    try {
      let model;
      if (name === 'openai') {
        model = createOpenAI({ apiKey: pConfig.apiKey, baseURL: pConfig.url }).chat(pConfig.model);
      } else if (name === 'anthropic') {
        model = createAnthropic({ apiKey: pConfig.apiKey, baseURL: pConfig.url })(pConfig.model);
      } else if (name === 'gemini' || name === 'google') {
        model = createGoogleGenerativeAI({ apiKey: pConfig.apiKey })(pConfig.model);
      } else if (name === 'deepseek') {
        model = createDeepSeek({ apiKey: pConfig.apiKey, baseURL: pConfig.url })(pConfig.model);
      ...
      const { text, toolCalls } = await generateText({
        ...
        toolChoice: (name === 'deepseek' && pConfig.model.includes('reasoner')) ? 'auto' : 'required'
      });

      if (toolCalls && toolCalls.length > 0) {
        console.log(`  - Result: SUCCESS! Tool Call: "${toolCalls[0].toolName}" with args: ${JSON.stringify((toolCalls[0] as any).args)}\n`);
      } else {
