...
import { runAgentLoop, AgentConfig } from '../../shared/lib/agent';
      ...
      <div className="flex-1 overflow-y-auto px-6 pt-4 pb-32 space-y-1.5 mask-fade-vertical z-10" ref={scrollRef}>
          ...
          <div className="text-muted-foreground flex flex-col items-center justify-center h-full space-y-6">
            <div className="space-y-3 text-sm opacity-80">
              ...
              <p className="flex items-center gap-3">
                ...
                Use <code className="text-primary font-bold">/skill</code> to mention AI skills
              </p>
              <p className="flex items-center gap-3">
                <span className="w-6 h-6 rounded bg-accent shrink-0 flex items-center justify-center text-xs font-mono font-bold">@</span>
                Use <code className="text-primary font-bold">@file</code> to reference files
              </p>
              <p className="flex items-center gap-3">
                <span className="w-6 h-6 rounded bg-accent shrink-0 flex items-center justify-center text-xs font-mono font-bold">&gt;</span>
                Use <code className="text-primary font-bold">&gt;cmd</code> to run custom commands
              </p>
            </div>
          </div>
        )}
        ...
        {messages.map((m, idx) => (
          <ChatMessageItem
            ...
            renderThought={(thought) => <ThoughtBlock content={thought} defaultExpanded={idx === messages.length - 1} />}
            renderToolCall={(call) => <ToolCallItem call={call} defaultExpanded={idx === messages.length - 1} />}
            renderToolResult={(name, content) => <ToolResultMessage name={name} content={content} defaultExpanded={idx === messages.length - 1} />}
          />
        ))}
        ...
        {loading && (
          <div className="flex items-center gap-2 text-muted-foreground animate-pulse ml-12 py-2">
            ...
            <span className="text-[10px] font-bold uppercase tracking-widest opacity-50 ml-1">Assistant is thinking...</span>
          </div>
        )}
      </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-background via-background to-transparent z-10">
        <div className="max-w-4xl mx-auto relative group">
          <ChatInput
            ...
            onShowHistory={() => { loadSessions(); setShowHistory(true); }}
          />
