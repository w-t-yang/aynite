...
export const calculateBollinger = (data: StockData[], period: number, multiplier: number) => {
...
export const calculateCCI = (data: StockData[], period: number = 20): number[] => {
...
export const calculateKeltner = (data: StockData[], period: number = 20, multiplier: number = 2) => {
...
export const enrichDataWithIndicators = (data: StockData[]): StockData[] => {
    ...
    const { middle: keltnerMiddle, upper: keltnerUpper, lower: keltnerLower } = calculateKeltner(data);

    return data.map((d, i) => ({
        ...d,
        sma5: isNaN(sma5[i]) ? undefined : sma5[i],
        sma10: isNaN(sma10[i]) ? undefined : sma10[i],
        sma20: isNaN(sma20[i]) ? undefined : sma20[i],
        sma50: isNaN(sma50[i]) ? undefined : sma50[i],
        sma100: isNaN(sma100[i]) ? undefined : sma100[i],
        ...
        ema200: isNaN(ema200[i]) ? undefined : ema200[i],
        vwap: vwap[i],
        bollingerUpper: isNaN(bbUpper[i]) ? undefined : bbUpper[i],
        bollingerLower: isNaN(bbLower[i]) ? undefined : bbLower[i],
        donchianHigh: isNaN(dcHigh[i]) ? undefined : dcHigh[i],
        donchianLow: isNaN(dcLow[i]) ? undefined : dcLow[i],
        macdLine: isNaN(macdLine[i]) ? undefined : macdLine[i],
        signalLine: isNaN(signalLine[i]) ? undefined : signalLine[i],
        histogram: isNaN(histogram[i]) ? undefined : histogram[i],
        rsi: isNaN(rsi[i]) ? undefined : rsi[i],
        atr: isNaN(atr[i]) ? undefined : atr[i],
        stochK: isNaN(stochK[i]) ? undefined : stochK[i],
        stochD: isNaN(stochD[i]) ? undefined : stochD[i],
        cci: isNaN(cci[i]) ? undefined : cci[i],
        sar: isNaN(sar[i]) ? undefined : sar[i],
        keltnerMiddle: isNaN(keltnerMiddle[i]) ? undefined : keltnerMiddle[i],
        keltnerUpper: isNaN(keltnerUpper[i]) ? undefined : keltnerUpper[i],
        keltnerLower: isNaN(keltnerLower[i]) ? undefined : keltnerLower[i],
    }));
};

export const aggregateData = (data: StockData[], interval: string): StockData[] => {
