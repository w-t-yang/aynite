...
import { useAppEvent, useView, useAppOperation } from '../ViewContext';
import { StockData, ChartComponentProps, DEFAULT_INDICATORS, TimeInterval } from './types';
import { enrichDataWithIndicators, aggregateData } from './indicators';
...
const CustomTooltip = ({ active, payload, label, theme, upColor, downColor }: any) => {
  if (!active || !payload || payload.length === 0) return null;

  const data = payload[0].payload;

  return (
    <div className={`p-3 border rounded shadow-xl backdrop-blur-md text-xs font-mono z-50
      ${theme === 'dark' ? 'bg-[#1e293b]/95 border-slate-700 text-slate-200' : 'bg-white/95 border-slate-200 text-slate-800'}
    `}>
      <div className="font-bold mb-2 border-b border-slate-500/20 pb-1 text-slate-500 dark:text-slate-400">{label}</div>

      <div className="grid grid-cols-2 gap-x-8 gap-y-1 mb-3">
        <div className="flex justify-between gap-4"><span className="text-slate-500 dark:text-slate-400">Open</span> <span>{data.open.toFixed(2)}</span></div>
        <div className="flex justify-between gap-4"><span className="text-slate-500 dark:text-slate-400">High</span> <span>{data.high.toFixed(2)}</span></div>
        <div className="flex justify-between gap-4"><span className="text-slate-500 dark:text-slate-400">Low</span> <span>{data.low.toFixed(2)}</span></div>
        <div className="flex justify-between gap-4"><span className="text-slate-500 dark:text-slate-400">Close</span> <span style={{ color: data.close >= data.open ? upColor : downColor }}>{data.close.toFixed(2)}</span></div>
      </div>
        ...
        {payload.map((entry: any, index: number) => {
          ...
          return (
            <div key={entry.name} className="flex justify-between items-center gap-4">
              <span style={{ color: entry.color }} className="flex items-center gap-1 font-semibold">
                {entry.name}
              </span>
...
export function StockChart() {
  ...
  const [error, setError] = useState<{ message: string; expected: string } | null>(null);
...
OR
{
  ...
  useAppEvent('chart-data', (payload: any) => {
    ...
    if (payload.indicators) setIndicators(prev => ({ ...prev, ...payload.indicators }));
  ...
  const handleSelectSystemFile = async () => {
    try {
      ...
      if (path) {
        const content = await (window as any).aynite.readFile(path);
        const json = JSON.parse(content);
        const { data: parsedData, symbol: parsedSymbol } = parseJsonData(json, symbol);
  ...
  const chartData = useMemo(() => {
    ...
    const comparePrices = compareData.slice(startIndex, startIndex + visibleCount).map(d => d.close);

    if (mainPrices.length === 0 || comparePrices.length === 0) return visibleData;
    ...
    return visibleData.map((d, i) => {
      ...
      const normalizedCompare = compareClose !== undefined
  ...
  const { dataMin, dataMax } = useMemo(() => {
    ...
    if (indicators.bollinger) {
      const bLower = Math.min(...chartData.filter(d => d.bollingerLower != null).map(d => d.bollingerLower as number));
      if (isFinite(bLower)) min = Math.min(min, bLower);
      const bUpper = Math.max(...chartData.filter(d => d.bollingerUpper != null).map(d => d.bollingerUpper as number));
      if (isFinite(bUpper)) max = Math.max(max, bUpper);
    }
  ...
  const macdDomain = useMemo(() => {
    ...
    const values = chartData.flatMap(d => [d.macdLine, d.signalLine, d.histogram]).filter(v => v != null) as number[];
  ...
  if (!data || data.length === 0) {
    return (
      <div className="w-full h-full flex items-center justify-center bg-background">
        <div className="text-muted-foreground text-sm animate-pulse">Waiting for Chart Data...</div>
      </div>
    );
  }
      ...
      <div className="h-10 border-b border-border flex items-center px-4 gap-4 bg-muted/30 justify-between shrink-0 relative z-30">
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-1 text-[10px] font-bold">
            {[TimeInterval.D1, TimeInterval.W1, TimeInterval.M1, TimeInterval.Y1].map((tf) => (
              <button
                type="button"
                key={tf}
            ...
            {isIndicatorMenuOpen && (
              <>
                ...
                <div className="absolute top-full left-0 mt-1 w-56 bg-popover border border-border rounded-lg shadow-xl z-20 overflow-hidden py-1 max-h-[70vh] overflow-y-auto backdrop-blur-md">
                  <div className="px-3 py-1.5 text-[10px] font-bold text-muted-foreground uppercase opacity-50">Trend & Moving Averages</div>
                  {[
                    { id: 'sma5', label: 'SMA 5' },
                    ...
                    >
                      {ind.label} {indicators[ind.id as keyof typeof indicators] && <Check size={12} className="text-primary" />}
                    </button>
                  ))}

                  <div className="border-t border-border my-1"></div>
                  <div className="px-3 py-1.5 text-[10px] font-bold text-muted-foreground uppercase opacity-50">Channels & Bands</div>
                  {[
                  ...
                  <div className="border-t border-border my-1"></div>
                  <div className="px-3 py-1.5 text-[10px] font-bold text-muted-foreground uppercase opacity-50">Oscillators</div>
                  {[
                    { id: 'macd', label: 'MACD' },
                    { id: 'rsi', label: 'RSI' },
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <Upload size={14} /> <span className="text-[10px] font-bold">LOAD JSON</span>
          </button>
        </div>
      </div>
      ...
      >
        ...
        {isMock && (
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none select-none overflow-hidden opacity-[0.03] dark:opacity-[0.05]">
            <span className="text-[15vw] font-black tracking-tighter rotate-12 whitespace-nowrap">SIMULATED DATA</span>
          </div>
        )}
        ...
        {error && (
          <div className="absolute inset-0 z-50 flex items-center justify-center p-6 bg-background/80 backdrop-blur-sm">
            <div className="max-w-xl w-full bg-popover border-2 border-destructive/50 rounded-xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
              ...
              <div className="p-4">
                <h4 className="text-xs font-bold text-muted-foreground mb-2 uppercase">Expected Format Examples:</h4>
        ...
        <div className="absolute top-4 left-4 z-10 pointer-events-none">
          <h2 className="text-3xl font-bold text-foreground tracking-tight flex items-center gap-2">
            {symbol}
            <span className="text-sm font-normal text-muted-foreground bg-muted px-2 py-0.5 rounded border border-border">{timeframe}</span>
          </h2>
          <div className="flex items-center gap-4 text-xs font-mono mt-1">
            <span className="text-muted-foreground">O <span style={{ color: latest.close >= latest.open ? COLOR_UP : COLOR_DOWN }}>{latest.open.toFixed(2)}</span></span>
            <span className="text-muted-foreground">H <span style={{ color: latest.close >= latest.open ? COLOR_UP : COLOR_DOWN }}>{latest.high.toFixed(2)}</span></span>
            <span className="text-muted-foreground">L <span style={{ color: latest.close >= latest.open ? COLOR_UP : COLOR_DOWN }}>{latest.low.toFixed(2)}</span></span>
            <span className="text-muted-foreground">C <span style={{ color: latest.close >= latest.open ? COLOR_UP : COLOR_DOWN }}>{latest.close.toFixed(2)}</span></span>
            <span className="text-muted-foreground">Vol <span className="text-foreground">{(latest.volume / 1000000).toFixed(2)}M</span></span>
          </div>
        </div>
        ...
        {compareSymbol && (
          <div className="absolute top-4 right-20 z-10 pointer-events-none">
            <div className="flex items-center gap-2 bg-muted/80 px-3 py-1 rounded-full border border-border">
              ...
              <span className="font-bold text-yellow-600 dark:text-yellow-400 text-xs">{compareSymbol}</span>
            </div>
          </div>
        )}
        ...
        <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={0} debounce={50}>
          <ComposedChart data={chartData} margin={{ top: 20, right: 0, left: 0, bottom: 0 }}>
            <CartesianGrid stroke={gridColor} vertical={true} horizontal={true} strokeDasharray="3 3" opacity={0.5} />
            ...
            <YAxis yAxisId="volume" orientation="left" domain={volumeDomain} hide />
            <YAxis yAxisId="macd" orientation="right" domain={macdDomain} hide />
            <YAxis yAxisId="rsi" orientation="right" domain={rsiDomain} hide />
            <YAxis yAxisId="stoch" orientation="right" domain={stochDomain} hide />
            ...
            <Tooltip
              ...
              content={<CustomTooltip theme={themeType} upColor={COLOR_UP} downColor={COLOR_DOWN} />}
            />
            ...
            >
              {chartData.map((entry, index) => (
                <Cell key={`cell-${entry.time}`} fill={entry.close >= entry.open ? COLOR_VOL_UP : COLOR_VOL_DOWN} />
              ))}
            </Bar>

            {indicators.macd && (
              <>
                <Bar dataKey="histogram" yAxisId="macd" name="Histogram" isAnimationActive={false} barSize={4}>
                  {chartData.map((entry, index) => (
                    <Cell key={`hist-${entry.time}`} fill={(entry.histogram || 0) >= 0 ? COLOR_UP : COLOR_DOWN} fillOpacity={0.5} />
                  ))}
                </Bar>
                <Line yAxisId="macd" type="monotone" dataKey="macdLine" stroke="#2962ff" dot={false} strokeWidth={1} isAnimationActive={false} name="MACD" />
                <Line yAxisId="macd" type="monotone" dataKey="signalLine" stroke="#ff6d00" dot={false} strokeWidth={1} isAnimationActive={false} name="Signal" />
              </>
            )}

            {indicators.rsi && (
              <>
                <Line yAxisId="rsi" type="monotone" dataKey="rsi" stroke="#a855f7" dot={false} strokeWidth={1.5} isAnimationActive={false} name="RSI" />
                <ReferenceLine yAxisId="rsi" y={70} stroke="#a855f7" strokeDasharray="3 3" strokeOpacity={0.5} />
                <ReferenceLine yAxisId="rsi" y={30} stroke="#a855f7" strokeDasharray="3 3" strokeOpacity={0.5} />
              </>
            )}

            {indicators.cci && (
              <>
                <Line yAxisId="cci" type="monotone" dataKey="cci" stroke="#06b6d4" dot={false} strokeWidth={1.5} isAnimationActive={false} name="CCI" />
                <ReferenceLine yAxisId="cci" y={100} stroke="#06b6d4" strokeDasharray="3 3" strokeOpacity={0.5} />
                <ReferenceLine yAxisId="cci" y={-100} stroke="#06b6d4" strokeDasharray="3 3" strokeOpacity={0.5} />
                <ReferenceLine yAxisId="cci" y={0} stroke="#06b6d4" strokeDasharray="3 3" strokeOpacity={0.3} />
              </>
            )}

            {indicators.atr && (
              <Line yAxisId="atr" type="monotone" dataKey="atr" stroke="#f43f5e" dot={false} strokeWidth={1.5} isAnimationActive={false} name="ATR" />
            )}

            {indicators.stoch && (
              <>
                <Line yAxisId="stoch" type="monotone" dataKey="stochK" stroke="#fb923c" dot={false} strokeWidth={1.5} isAnimationActive={false} name="%K" />
                <Line yAxisId="stoch" type="monotone" dataKey="stochD" stroke="#fdba74" dot={false} strokeWidth={1.5} isAnimationActive={false} name="%D" />
                <ReferenceLine yAxisId="stoch" y={80} stroke="#fb923c" strokeDasharray="3 3" strokeOpacity={0.5} />
                <ReferenceLine yAxisId="stoch" y={20} stroke="#fb923c" strokeDasharray="3 3" strokeOpacity={0.5} />
              </>
            )}

            {indicators.bollinger && (
              <>
                <Line yAxisId="price" type="monotone" dataKey="bollingerUpper" stroke="#8b5cf6" strokeOpacity={0.5} dot={false} strokeWidth={1} isAnimationActive={false} />
                <Line yAxisId="price" type="monotone" dataKey="bollingerLower" stroke="#8b5cf6" strokeOpacity={0.5} dot={false} strokeWidth={1} isAnimationActive={false} />
              </>
            )}
            ...
            <Bar
              ...
              yAxisId="price"
              shape={<CandlestickShape upColor={COLOR_UP} downColor={COLOR_DOWN} />}
              isAnimationActive={false}
              name="Price"
              barSize={6}
            />

            {indicators.sma5 && <Line yAxisId="price" type="monotone" dataKey="sma5" stroke="#06b6d4" dot={false} strokeWidth={1.5} isAnimationActive={false} name="SMA 5" />}
            {indicators.sma10 && <Line yAxisId="price" type="monotone" dataKey="sma10" stroke="#0ea5e9" dot={false} strokeWidth={1.5} isAnimationActive={false} name="SMA 10" />}
            {indicators.sma20 && <Line yAxisId="price" type="monotone" dataKey="sma20" stroke="#2962ff" dot={false} strokeWidth={1.5} isAnimationActive={false} name="SMA 20" />}
            {indicators.sma50 && <Line yAxisId="price" type="monotone" dataKey="sma50" stroke="#6366f1" dot={false} strokeWidth={1.5} isAnimationActive={false} name="SMA 50" />}
            {indicators.sma100 && <Line yAxisId="price" type="monotone" dataKey="sma100" stroke="#22c55e" dot={false} strokeWidth={1.5} isAnimationActive={false} name="SMA 100" />}
            {indicators.sma200 && <Line yAxisId="price" type="monotone" dataKey="sma200" stroke="#10b981" dot={false} strokeWidth={1.5} isAnimationActive={false} name="SMA 200" />}

            {indicators.ema12 && <Line yAxisId="price" type="monotone" dataKey="ema12" stroke="#eab308" dot={false} strokeWidth={1.5} isAnimationActive={false} name="EMA 12" />}
            {indicators.ema26 && <Line yAxisId="price" type="monotone" dataKey="ema26" stroke="#ff9800" dot={false} strokeWidth={1.5} isAnimationActive={false} name="EMA 26" />}
            {indicators.ema50 && <Line yAxisId="price" type="monotone" dataKey="ema50" stroke="#f59e0b" dot={false} strokeWidth={1.5} isAnimationActive={false} name="EMA 50" />}
            {indicators.ema200 && <Line yAxisId="price" type="monotone" dataKey="ema200" stroke="#ef4444" dot={false} strokeWidth={1.5} isAnimationActive={false} name="EMA 200" />}

            {indicators.vwap && <Line yAxisId="price" type="monotone" dataKey="vwap" stroke="#e11d48" dot={false} strokeWidth={1.5} strokeDasharray="3 3" isAnimationActive={false} name="VWAP" />}

            {indicators.donchian && (
              <>
                <Line yAxisId="price" type="monotone" dataKey="donchianHigh" stroke="#0ea5e9" dot={false} strokeWidth={1} strokeDasharray="2 2" isAnimationActive={false} name="DC High" />
                <Line yAxisId="price" type="monotone" dataKey="donchianLow" stroke="#0ea5e9" dot={false} strokeWidth={1} strokeDasharray="2 2" isAnimationActive={false} name="DC Low" />
              </>
            )}

            {indicators.keltner && (
              <>
                <Line yAxisId="price" type="monotone" dataKey="keltnerMiddle" stroke="#14b8a6" dot={false} strokeWidth={1} isAnimationActive={false} name="Keltner Mid" />
                <Line yAxisId="price" type="monotone" dataKey="keltnerUpper" stroke="#14b8a6" dot={false} strokeWidth={1} strokeDasharray="2 2" isAnimationActive={false} name="Keltner Upper" />
                <Line yAxisId="price" type="monotone" dataKey="keltnerLower" stroke="#14b8a6" dot={false} strokeWidth={1} strokeDasharray="2 2" isAnimationActive={false} name="Keltner Lower" />
              </>
            )}

            {indicators.sar && <Scatter yAxisId="price" dataKey="sar" fill="#3b82f6" shape="circle" isAnimationActive={false} name="SAR" />}

            {compareData && (
              <Line yAxisId="price" type="monotone" dataKey="compareClose" stroke="#facc15" dot={false} strokeWidth={2} isAnimationActive={false} name={compareSymbol || 'Compare'} />
            )}

            <ReferenceLine yAxisId="price" y={latest.close} stroke={latest.close >= latest.open ? COLOR_UP : COLOR_DOWN} strokeDasharray="3 3" strokeOpacity={0.5} />
          </ComposedChart>
        </ResponsiveContainer>
      </section>
    </div>
