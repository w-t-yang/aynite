...
const API = {
  ...SettingsMock,
  getDefaults: () => demoDefaults
};
import { cn } from '../../src/lib/utils';
import { SearchableSelect } from '../../src/components/ui/SearchableSelect';
import { KeyManager } from '../../src/lib/key-handlers';
import { AIProviderInstance, SettingsState } from '../../src/lib/types';
import { UnifiedCollapsible } from '../../src/components/Chat';
...
export default function Settings({ settings, onSave, onClose }: SettingsProps) {
  ...
  useEffect(() => {
    ...
    const unsubChecking = API.onUpdateChecking(() => setUpdateStatus('checking'));
    const unsubAvailable = (info: any) => {
      setUpdateStatus('available');
      setUpdateInfo(info);
    };
    const offAvailable = API.onUpdateAvailable(unsubAvailable);

    const unsubNotAvailable = API.onUpdateNotAvailable(() => setUpdateStatus('idle'));
    ...
    const offProgress = API.onUpdateProgress(unsubProgress);

    const unsubDownloaded = (info: any) => {
      setUpdateStatus('downloaded');
      setUpdateInfo(info);
    };
    const offDownloaded = API.onUpdateDownloaded(unsubDownloaded);
  ...
  useEffect(() => {
    if (activeTab === 'agents') {
      ...
      // @ts-ignore
      API.getMergedSystemPrompt(localSettings.prompts?.files, activeAgent?.promptFiles).then(res => {
        if (res?.data) setMergedPrompt(res.data);
      });
    }
    if (activeTab === 'skills') {
      // @ts-ignore
      API.getAvailableSkills().then(res => {
        if (res?.data) setAvailableSkills(res.data);
      });
    }
    if (activeTab === 'commands') {
      // @ts-ignore
      API.getAvailableCommands().then(res => {
    ...
    const newProviders = (localSettings.ai?.providers || []).map(p => {
      if (p.id === id) {
        ...
        if (field === 'provider' || field === 'model') {
          ...
          if (field === 'provider') {
            updated.model = (API.getDefaults().providerModels as Record<string, string>)[value] || updated.model;
            updated.url = (API.getDefaults().providerUrls as Record<string, string>)[value] !== undefined ? (API.getDefaults().providerUrls as Record<string, string>)[value] : updated.url;
          }
          ...
          const providerLabel = ({
            ...
            deepseek: 'DeepSeek',
            others: 'Compatible'
          } as Record<string, string>)[updated.provider] || updated.provider;
  ...
  useEffect(() => {
    loadThemes();
    // @ts-ignore
    API.getSystemFonts().then((res: any) => { if (res?.data) setSystemFonts(res.data); });
  }, []);
  ...
  const persistTheme = useCallback((theme: any) => {
    // @ts-ignore
    API.saveTheme(theme.id, { name: theme.name, type: theme.type, isSystem: theme.isSystem, colors: theme.colors, fonts: theme.fonts });
  ...
  const handleRestore = async () => {
    if (!editingTheme) return;
    // @ts-ignore
    await API.restoreDefaultTheme(editingTheme.id);
  ...
  const handleDelete = async () => {
    ...
    // @ts-ignore
    await API.deleteTheme(editingTheme.id);
    await loadThemes();
  ...
  const handleDuplicate = async () => {
    ...
    // @ts-ignore
    await API.saveTheme(id, newTheme);
    await loadThemes();
            ...
            {activeTab !== 'about' && (
              <div className="flex items-center justify-between mb-6">
                ...
                {activeTab === 'keybindings' && (
                  <button onClick={() => {
                    save({ ...localSettings, keybindings: API.getDefaults().keybindings });
              ...
              <div className="space-y-10 max-w-4xl pb-10">
                <div className="flex items-center justify-between border-b border-border/30 pb-4">
                  ...
                  <button onClick={async () => {
                    // @ts-ignore
                    const res = await API.restoreDefaultPrompts();
                    if (res && res.data) {
                      save({
                        ...localSettings,
                ...
                <div>
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="flex items-center gap-2">
                      <button onClick={async () => {
                        // @ts-ignore
                        const res = await API.pickPromptFile();
                        if (res && res.data) {
              ...
              <div className="space-y-6 max-w-4xl">
                ...
                <div>
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <button onClick={async () => {
                      // @ts-ignore
                      const res = await API.pickSkillFolder();
                      if (res && res.data) {
                      ...
                      <div key={folder} className="flex items-center justify-between p-3 rounded-lg border border-border bg-accent/10 group">
                        ...
                        <div className="flex items-center gap-2">
                          {index === 0 && (
                            <button onClick={async () => {
                              // @ts-ignore
                              const res = await API.restoreDefaultSkills();
              ...
              <div className="space-y-6 max-w-4xl">
                ...
                <div>
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <button onClick={async () => {
                      // @ts-ignore
                      const res = await API.pickCommandFolder();
                      if (res && res.data) {
                      ...
                      <div key={folder} className="flex items-center justify-between p-3 rounded-lg border border-border bg-accent/10 group">
                        ...
                        <div className="flex items-center gap-2">
                          {index === 0 && (
                            <button onClick={async () => {
                              // @ts-ignore
                              const res = await API.restoreDefaultCommands();
                  ...
                  <div className="flex items-center justify-between mb-6">
                    ...
                    <button
                      onClick={() => {
                        save({
                          ...localSettings,
                          aiTools: { ...API.getDefaults().aiTools }
                        });
                ...
                <div className="grid grid-cols-1 gap-4 pt-4">
                  ...
                  <div className="p-6 rounded-2xl border border-border bg-accent/5 space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <button onClick={() => API.openExternal('https://github.com/w-t-yang/aynite')} className="flex items-center gap-2 p-2 hover:bg-accent rounded-lg text-xs text-muted-foreground hover:text-foreground transition-colors border border-transparent hover:border-border">
                        <Github size={14} /> GitHub Project
                      </button>
                      <button onClick={() => API.openExternal('https://github.com/w-t-yang/aynite/issues')} className="flex items-center gap-2 p-2 hover:bg-accent rounded-lg text-xs text-muted-foreground hover:text-foreground transition-colors border border-transparent hover:border-border">
