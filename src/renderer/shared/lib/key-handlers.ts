...
export class KeyManager {
  ...
  static init(settings: any, globalApi: GlobalAPI) {
    KeyManager.settings = settings
    KeyManager.globalApi = globalApi

    if (!KeyManager.isInitialized) {
      window.addEventListener('keydown', KeyManager.handleGlobalKeyDown, true)
      window.addEventListener('keyup', KeyManager.handleGlobalKeyUp, true)
      KeyManager.isInitialized = true
    }
  }

  static cleanup() {
    window.removeEventListener('keydown', KeyManager.handleGlobalKeyDown, true)
    window.removeEventListener('keyup', KeyManager.handleGlobalKeyUp, true)
    KeyManager.isInitialized = false
  }

  static setActiveTab(tabId: string | null) {
    KeyManager.activeTabId = tabId
  }

  static registerEditor(tabId: string, api: EditorAPI) {
    KeyManager.tabApis.set(tabId, api)
  }

  static unregisterEditor(tabId: string) {
    KeyManager.tabApis.delete(tabId)
  }

  static registerTabSwitcher(api: TabSwitcherAPI) {
    KeyManager.tabSwitcherApi = api
  }

  static unregisterTabSwitcher() {
    KeyManager.tabSwitcherApi = null
  }

  static registerSettings(api: SettingsAPI) {
    KeyManager.settingsApi = api
  }

  static unregisterSettings() {
    KeyManager.settingsApi = null
  }

  static registerSidebar(api: SidebarAPI) {
    KeyManager.sidebarApi = api
  }

  static unregisterSidebar() {
    KeyManager.sidebarApi = null
  }

  static registerChat(api: ChatAPI) {
    KeyManager.chatApi = api
  }

  static unregisterChat() {
    KeyManager.chatApi = null
  }

  private static checkMatch(
    e: KeyboardEvent,
  ...
  private static handleGlobalKeyDown = (e: KeyboardEvent) => {
    ...
    if (isCmd && key === 'S') {
      e.preventDefault()
      this.globalApi.saveActiveTab()
      return
    }

    if (this.checkMatch(e, kb.app[AppOperation.REFRESH_APP])) {
      e.preventDefault()
      this.globalApi.reload()
      return
    }
    if (this.checkMatch(e, kb.app[AppOperation.TOGGLE_LEFT_PANEL])) {
      e.preventDefault()
      this.globalApi.toggleLeftPanel()
      return
    }
    if (this.checkMatch(e, kb.app[AppOperation.TOGGLE_RIGHT_PANEL])) {
      e.preventDefault()
      this.globalApi.toggleRightPanel()
      return
    }
    if (this.checkMatch(e, kb.app[AppOperation.FOCUS_CHAT])) {
      e.preventDefault()
      this.globalApi.focusChat()
      return
    }
    if (this.checkMatch(e, kb.app[AppOperation.FOCUS_SKILLS])) {
      e.preventDefault()
      this.globalApi.focusSkills()
      return
    }
    if (this.checkMatch(e, kb.app[AppOperation.FOCUS_COMMANDS])) {
      e.preventDefault()
      this.globalApi.focusCommands()
      return
    }
    ...
    if (
      context === 'editor' &&
      this.activeTabId &&
      this.tabApis.has(this.activeTabId)
    ) {
      const handled = this.handleEditorKeys(
        e,
        this.tabApis.get(this.activeTabId)!,
        kb,
      )
      if (handled) return
    }
  ...
  private static handleEditorKeys = (
    e: KeyboardEvent,
    api: EditorAPI,
    kb: any,
  ): boolean => {
    ...
    if (
      api.isEditing() &&
      (e.key === 'Escape' || this.checkMatch(e, v[ViewOperation.KEYBOARD_QUIT]))
    ) {
      e.preventDefault()
  ...
  private static handleGlobalKeyUp = (_e: KeyboardEvent) => {
