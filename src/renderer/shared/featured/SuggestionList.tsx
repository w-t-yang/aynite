...
>(({ items, command, triggerChar }, ref) => {
  ...
  const selectionItems: SelectionItem[] = items.map((item) => {
    let icon = <FileText key={item.id} size={14} />
    if (item.error)
      icon = <AlertTriangle key={item.id} size={14} className="text-warning" />
    else if (triggerChar === '/') icon = <Zap key={item.id} size={14} />
    else if (triggerChar === '>') icon = <Terminal key={item.id} size={14} />
    else if (item.isDirectory) icon = <Folder key={item.id} size={14} />
