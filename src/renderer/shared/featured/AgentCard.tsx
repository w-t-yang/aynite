...
const DELETE_BTN_CLASS =
  'p-1.5 text-muted-foreground hover:text-destructive hover:bg-destructive/10 rounded transition-all opacity-0 group-hover:opacity-100'
const ROW_CLASS =
  'flex items-center justify-between p-2.5 rounded-lg border border-border bg-accent/5 group'
  ...
  return (
    <div className={ROW_CLASS}>
      <div className={FLEX_COL_MIN}>
        <span className={FILE_NAME_CLASS}>{filePath.split(/[/\\]/).pop()}</span>
        <span className={FILE_PATH_CLASS}>{filePath}</span>
      </div>
      <Button
        variant="ghost"
        size="icon"
      ...
      <EditableCardFrame isActive={isActive}>
        ...
        <div className="ml-7 space-y-4">
          <div className={FLEX_CENTER_BETWEEN}>
            <h4 className={SECTION_LABEL}>Agent Prompt Files</h4>
            <Button
              variant="ghost"
              size="sm"
