...
export const HtmlViewer: React.FC<{ file: FileInfo; content?: string }> = ({ file, content }) => {
  ...
  const themeVars = [
    '--background', '--foreground', '--sidebar', '--border', '--primary', 
    '--accent', '--muted', '--muted-foreground', '--info', '--success', '--warning'
  ...
  if (finalContent.includes('<head>')) {
    ...
    finalContent = finalContent.replace('<html>', `<html><head>${inject}</head>`);
  } else {
