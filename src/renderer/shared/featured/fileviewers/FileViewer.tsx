...
function FileViewer({
  ...
  const [_scrollPos, setScrollPos] = useState({ top: 0, left: 0 })
  ...
  const category = fileInfo
    ? getFileCategory(fileInfo.extension, fileInfo.isText, fileInfo.path)
    : 'text'
  ...
  }, [fileInfo?.path, effectiveCategory, isTooLarge, fileInfo, category])
    ...
    (data) => {
      ...
      if (
        ...
        if (now - lastLocalWriteTime.current < 2000) {
          console.log(
            '[FileViewer] Ignoring change event likely triggered by local save',
          )
          return
        }
  ...
  const _handleContentChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    undoStack.current.push(localContent)
  ...
  useEffect(() => {
    const api = {
      ...
      isSearchInputFocused: (target: EventTarget | null) =>
      ...
      killLine: () => {
        ...
        if (textarea) {
          ...
          const newContent =
            ...
            localContent.substring(nextLineStart)
          setLocalContent(newContent)
          if (onChange) {
            isLocalChange.current = true
            onChange(newContent)
  ...
  const _handleScroll = (e: React.UIEvent<HTMLTextAreaElement>) => {
