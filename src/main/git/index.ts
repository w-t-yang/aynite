    ...
    ipcMain.handle(
      ...
      async (_event, root: string) => {
        try {
          ...
          const systemPrompt = `You are a commit message generator. Generate a concise git commit message in conventional commit format.

Format rules:
- First line: {type}: {short summary}
- Then a blank line
- Then bullet points listing specific changes
- Last line must be exactly: Generated by Aynite (${modelName})

Type must be one of: feat, fix, chore, refactor, docs, test, style.
